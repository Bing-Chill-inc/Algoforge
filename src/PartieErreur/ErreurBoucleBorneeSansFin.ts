 ...
 */
export class ErreurBoucleBorneeSansFin extends ErreurConceptuelle {
	// ATTRIBUTS  -- Non --
	 ...
	 */
	static detecterAnomalie(uneBoucleBornee: this) {
		try {
			...
			if (
				...
				const borneInferieure = Number(uneBoucleBornee._borneInferieure);
				const borneSuperieure = Number(uneBoucleBornee._borneSuperieure);
				const pas = Number(uneBoucleBornee._pas);
