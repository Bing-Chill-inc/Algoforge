 ...
 */
export class ErreurDonneeMagique extends ErreurConceptuelle {
	// ATTRIBUTS
	...
	static detecterAnomalie(unProbleme: this) {
		...
		try {
			...
			let informationDesAntescedants: any[] = [];
			for (let parents of antescedents) {
				informationDesAntescedants = [
					...informationDesAntescedants,
					...parents.getInformationDonnee(),
				];
			}
			for (let uneInfomationEnDonnee of informationEnDonnee) {
				for (let informationAntescedants of informationDesAntescedants) {
					if (
						informationAntescedants._nom ==
						uneInfomationEnDonnee._nom
					) {
						informationEnDonnee = informationEnDonnee.filter(
							(uneInfo: { _nom: any; }) =>
								uneInfo._nom != informationAntescedants._nom,
						);
					}
				}
			}
			...
			for (let enfants of unProbleme.getParent().getEnfants()) {
				...
				for (let uneInfomationEnDonnee of informationEnDonnee) {
					for (let informationEnfant of enfants.getInformationResultat()) {
						if (
							informationEnfant._nom == uneInfomationEnDonnee._nom
						) {
							informationEnDonnee = informationEnDonnee.filter(
								(uneInfo: { _nom: any; }) =>
									uneInfo._nom != informationEnfant._nom,
							);
						}
					}
				}
