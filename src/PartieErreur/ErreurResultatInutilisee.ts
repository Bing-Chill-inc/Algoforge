 ...
 */
export class ErreurResultatInutilisee extends ErreurConceptuelle {
	// ATTRIBUTS
	__nomsResultats!: any[]; // Array<String>
	 ...
	 */
	constructor(elementEmetteur: any, nomsResultats = new Array()) {
	 ...
	 */
	set _nomsResultats(value: any[]) {
		this.__nomsResultats = value;
	}
	/**
	...
	toString() {
		if (this._nomsResultats.length == 1) {
			return (
				"Le résultat " + this._nomsResultats[0] + " n'est pas utilisé"
			);
		} else {
			let chaine = "";
		...
		try {
			...
			for (let antescedant of unProbleme.getAntescedants()) {
				for (let resultat of antescedant.getInformationResultat()) {
					resultatProbleme = resultatProbleme.filter(
						(unresultat: { _nom: any; }) => unresultat._nom != resultat._nom,
					);
				}
			}
			...
			for (let enfant of parent.getEnfants()) {
				...
				for (let donnee of enfant.getInformationDonnee()) {
					resultatProbleme = resultatProbleme.filter(
						(unresultat: { _nom: any; }) => unresultat._nom != donnee._nom,
					);
				}
			}
