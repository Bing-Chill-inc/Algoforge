 ...
 */
export class ErreurTypesInconsistantsAlternatif extends ErreurConceptuelle {
	// ATTRIBUTS
	__nomVariable!: String; // String
	__typePris!: any[]; // Array<Type>
	 ...
	 */
	set _typePris(value: any[]) {
		this.__typePris = value;
	}
	/**
	 ...
	 */
	static detecterAnomalie(uneStructureAlternative: this|this) {
		try {
			let types = [];
			let variable = null;
			if (uneStructureAlternative instanceof classes.StructureSi) {
				const regex = /^(.*?)\s*([=!<>]=?)\s*(.*?)$/;
				const premierLibelle =
					uneStructureAlternative._listeConditions.children[0]
						._libelle;
				let resultat = premierLibelle.match(regex);
				if (resultat) {
					variable = resultat[1];
					let typePremier =
						ErreurTypesInconsistantsAlternatif.determinerType(
							ErreurTypesInconsistantsAlternatif.extraireValeurComparaison(
								uneStructureAlternative._listeConditions
									.children[0]._libelle,
							),
						);
					types.push(typePremier);
					for (let condition of uneStructureAlternative
						._listeConditions.children) {
						let type =
							ErreurTypesInconsistantsAlternatif.determinerType(
								ErreurTypesInconsistantsAlternatif.extraireValeurComparaison(
									condition._libelle,
	 ...
	 */
	static extraireValeurComparaison(comparaison: string) {
	 ...
	 */
	static determinerType(texte: string|number|null) {
		if (!isNaN(texte)) {
			return Number;
		} else if (
			texte!.toLowerCase() == "true" ||
			texte!.toLowerCase() == "false"
		) {
			return Boolean;
		} else {
