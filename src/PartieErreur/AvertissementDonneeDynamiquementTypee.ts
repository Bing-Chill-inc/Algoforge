import { AvertissementConceptuel } from "./AvertissementConceptuel";
 ...
 */
export class AvertissementDonneeDynamiquementTypee extends AvertissementConceptuel {
	// ATTRIBUTS
	__nomsVariables!: any[]; // array<String>
	 ...
	 */
	constructor(elementEmetteur: any, nomsVariables = new Array()) {
	 ...
	 */
	set _nomsVariables(value: any[]) {
		this.__nomsVariables = value;
	}
	/**
		...
		try {
			...
			for (let information of lesInformations) {
				if (
					unProbleme
						.getPlanTravail()
						.leDictionnaireDesDonnees.containInformation(
							information._nom,
						)
				) {
					const informationDictionnaire = document
						.getElementById("espace1")!
						.leDictionnaireDesDonnees.getInformation(
							information._nom,
						);
					const informationBienType = document
						.getElementById("espace1")!
						.leDictionnaireDesDonnees.TypeCompatible(
							informationDictionnaire._type,
							information._type,
						);
					if (informationBienType) {
						lesInformationsPasTypeCorrectement =
							lesInformationsPasTypeCorrectement.filter(
								(uneinformation: { _nom: any; }) =>
									uneinformation._nom != information._nom,
							);
					}
				}
			}
	...
	toString() {
		if (this._nomsVariables.length == 1) {
			return (
				"La variable " +
				this._nomsVariables[0] +
				" est dynamiquement typée."
			);
		} else {
