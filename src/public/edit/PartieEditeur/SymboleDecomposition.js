...
class SymboleDecomposition extends HTMLElement {
    ...
    update() {
        this.style.left = "calc(var(--sizeModifier) * " + (this._pointAncrageX - 0.6) + "vw)";
        this.style.top = "calc(var(--sizeModifier) * " + (this._pointAncrageY) + "vw)";
    }

} window.customElements.define("symbole-decomposition-element", SymboleDecomposition);