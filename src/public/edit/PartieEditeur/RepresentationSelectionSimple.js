...
class RepresentationSelectionSimple extends HTMLElement {
    ...
    update() {
        if (this._element instanceof Probleme || this._element instanceof Procedure) {
            this.style.left = "calc(var(--sizeModifier) * " + (parseFloat(this._element._abscisse) + 10) + 'vw)';
            this.style.top = "calc(var(--sizeModifier) * " + parseFloat(this._element._ordonnee) + 'vw)';
            this.style.width = "calc(var(--sizeModifier) * " + (this._element.getTailleAbscisse() - 20) + 'vw)';
            this.style.height = "calc(var(--sizeModifier) * " + (this._element.getTailleOrdonnee() - 1) + 'vw)';
        } else if (this._element instanceof StructureAlternative) {
            this.style.left = "calc(var(--sizeModifier) * " + (parseFloat(this._element._abscisse) - 2.5) + 'vw)';
            this.style.top = "calc(var(--sizeModifier) * " + parseFloat(this._element._ordonnee) + 'vw)';
            this.style.width = "calc(var(--sizeModifier) * " + (this._element.getTailleAbscisse() + 6) + 'vw)';
            this.style.height = "calc(var(--sizeModifier) * " + (this._element.getTailleOrdonnee() + 2) + 'vw)';
        } else if (this._element instanceof StructureIterative) {
            this.style.left = "calc(var(--sizeModifier) * " + (parseFloat(this._element._abscisse)) + 'vw)';
            this.style.top = "calc(var(--sizeModifier) * " + parseFloat(this._element._ordonnee) + 'vw)';
            this.style.width = "calc(var(--sizeModifier) * " + (this._element.getTailleAbscisse()) + 'vw)';
