 ...
 * @extends {HTMLElement}
 */
export class IndicateurZoom extends HTMLElement {
	// ATTRIBUTS
	_editeur = editeur; // Editeur
	__zoom = parseFloat(this._editeur.getCookie("zoom"))
		? parseFloat(this._editeur.getCookie("zoom"))
		: 1; // Number
buttonZoomOut: HTMLButtonElement;
display: HTMLDivElement;
buttonZoomIn: HTMLButtonElement;
	...
	constructor() {
		...
		this.buttonZoomOut.addEventListener("click", (e: any) => {
			this.zoomOut();
		});
		...
		this.display.addEventListener("keydown", (e: { key: string; preventDefault: () => void; }) => {
			if (e.key === "Enter") {
				e.preventDefault();
				this.display.blur();
			}
		...
		this.buttonZoomIn.addEventListener("click", (e: any) => {
			this.zoomIn();
		});
