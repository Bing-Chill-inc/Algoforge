...
class Probleme extends ElementGraphique {
  ...
  afficher() {
    ...
    this.divNom.addEventListener("focusout", (e) => {
      ...
      this._editeur.ajouterEvenement(
        new EvenementEditionLibelleProbleme(this, this._libelle, this.libelle)
      );
  ...
  decomposerAutrePlan() {
    ...
    if (this._sousPlan != null) {
      ...
      const appliquerDecalage = (elem) => {
        elem.abscisse =
          parseFloat(elem.abscisse) - parseFloat(this._abscisse) + "vw";
        elem.ordonnee =
          parseFloat(elem.ordonnee) - parseFloat(this._ordonnee) + "vw";
        if (elem.enfants) {
