 ...
 */
export class StructureAlternative extends ElementGraphique {
	// ATTRIBUTS
	__listeConditions!: never[]; // HTML Collection de Condition•s
	...
	constructor(abscisse: number|undefined, ordonnee: number|undefined, listeConditions = []) {
		...
		if (this._listeConditions.length == 0) {
			this._listeConditions.push(new classes.Condition());
		...
		this.addEventListener("mousemove", function (e) {
			...
			for (let condition of this._listeConditions.children) {
				condition._elemParent._listeElementsEnfants.forEach((lien: { ligne: { update: () => void; }; }) => {
					lien.ligne.update();
				});
			}
		});
	 ...
	 */
	set _listeConditions(value: never[]) {
		this.__listeConditions = value;
	}
	...
	getEnfants(typeRechercher = classes.ElementGraphique) {
		...
		listeEnfants = classes.PlanTravail.FiltrerElementsGraphique(
			listeEnfants,
			typeRechercher,
		);
		return classes.PlanTravail.trierElementsGraphique(listeEnfants);
	}
	 ...
	 */
	ajouterCondition(condition = new classes.Condition()) {
		condition._structure = this;
		this._listeConditions.appendChild(condition);
		this._editeur.ajouterEvenement(
			new classes.EvenementCreationElement(condition, this._listeConditions),
		);
	}
	...
	supprimerCondition(condition = null) {
		...
		if (condition instanceof classes.Condition) {
			this._editeur.ajouterEvenement(
				new classes.EvenementSuppressionElement(condition),
			);
	 ...
	 */
	decalerCondition(pCondition: Node|null, decalage: number, estAnnulation = false) {
		if (!estAnnulation)
			this._editeur.ajouterEvenement(
				new classes.EvenementDeplacementCondition(pCondition, decalage),
			);
		if (decalage == -1 && pCondition instanceof classes.Condition) {
			...
			if (autreCondition != null) {
				...
				setTimeout(() => {
					...
					autreCondition._elemParent._listeElementsEnfants.forEach(
						(lien: { ligne: { update: () => void; }; }) => {
							lien.ligne.update();
						},
					);
				}, parseFloat(document.body.style.getPropertyValue("--specialTransitionTime")) * 1000);
			}
		} else if (decalage == 1 && pCondition instanceof classes.Condition) {
	 ...
	 */
	ajouterConditionParRapportA(pCondition: { nextElementSibling: any; _elemParent: { _listeElementsEnfants: any[]; }; }, decalage: number, estAnnulation = false) {
		if (decalage == -1 && pCondition instanceof classes.Condition) {
			// Ajouter une condition à gauche
			let newCondition = new classes.Condition();
		...
		setTimeout(() => {
			// update les liens vers les enfants
			pCondition._elemParent._listeElementsEnfants.forEach((lien: { ligne: { update: () => void; }; }) => {
				lien.ligne.update();
			});
		}, 200);
	}
	...
	getEnfantsParCondition() {
		...
		for (let condition of this._listeConditions.children) {
			...
			listeEnfantsParCondition = classes.PlanTravail.trierElementsGraphique(
	...
	genererOptionsContextuelles(editeur: any) {
		...
		listeOptions.push(
			new classes.ElementMenu("Délier toutes les conditions", () => {
