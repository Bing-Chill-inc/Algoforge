...
class SelectionRectangle extends HTMLElement {
    ...
    listerElementsGraphiques() {
        ...
        for (let element of this.parentNode.children) {
            if (verbose) console.log(`L'élément ${element.constructor.name} a pour abscisse ${element._abscisse} et pour ordonnée ${element._ordonnee}`);
            if (verbose) console.log(`Le rectangle a pour x1 ${this._x1} et pour y1 ${this._y1}`);
            if (verbose) console.log(`Le rectangle a pour x2 ${this._x2} et pour y2 ${this._y2}`);
            if (element instanceof ElementGraphique) {
                if (this._x1 <= parseFloat(element.selectAnchor.x) && 
