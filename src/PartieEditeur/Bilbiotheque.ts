 ...
 * @extends HTMLElement
 */
export class Bibliotheque extends HTMLElement {
	_estOuvert = false;
	_arborescence = null;
	_arborescenceCustom = [];
	 ...
	 */
	ajouterAlgorithmeCustom(nom: any, algo: any, descriptif: any, nomCourt: any) {
	...
	ouvrir() {
		...
		this._arborescence!.forEach((categorie: any) => {
	 ...
	 * @private
	 */
	_creerCategorieElement(categorie: { nom: any; contenu: any; }) {
	 ...
	 * @private
	 */
	_creerTitreCategorie(nom: string) {
	 ...
	 * @private
	 */
	_creerAlgorithmeElement(algorithme: { algo: any; descriptif: any; nom: string; nomCourt: string; }) {
	 ...
	 * @private
	 */
	_creerPreview(algorithme: { algo: string; nom: string; descriptif: string; }) {
		...
		try {
			const planTravail = new classes.PlanTravail();
	 ...
	 * @private
	 */
	_transformerAlgorithmeEnRelatif(algo: string) {
		const algoElements = JSON.parse(algo);

		let minY = Infinity;
		let coordCentreElement = { x: 0, y: 0 };
		algoElements.forEach((element: { ordonnee: string; abscisse: string; }) => {
		...
		const appliquerDecalage = (element: { abscisse: string; ordonnee: string; enfants: any[]; typeElement: string; conditions: any[]; }) => {
			element.abscisse =
			...
			if (element.enfants) {
				element.enfants.forEach((enfant: any) => appliquerDecalage(enfant));
			}
			...
			if (
				...
				element.conditions.forEach((condition: { enfants: any[]; }) => {
		...
		algoElements.forEach((element: any) => appliquerDecalage(element));
	 ...
	 * @private
	 */
	_ajouterEvenementsAlgorithme(algorithmeElement: HTMLDivElement, algoElements: any) {
		let previewTimeout: number|undefined;

		algorithmeElement.addEventListener("dragstart", (event: { dataTransfer: { setData: (arg0: string,arg1: string) => void; }; }) => {
		...
		algorithmeElement.addEventListener("dragend", (event: any) => {
			if (verbose) console.log(event);
			this.style.opacity = 1;
		});

		algorithmeElement.addEventListener("mouseenter", (event: any) => {
			if (verbose) console.log(event);
			clearTimeout(previewTimeout);
			this.appendChild(algorithmeElement.preview);
		});

		algorithmeElement.addEventListener("mouseleave", (event: any) => {
			if (verbose) console.log(event);
	 ...
	 * @private
	 */
	_creerAlgorithmeElementPersonnalise(algorithme: never) {
		const algorithmeElement = document.createElement("div");
