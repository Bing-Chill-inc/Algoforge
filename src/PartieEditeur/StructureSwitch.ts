 ...
 */
export class StructureSwitch extends StructureAlternative {
	static readonly typeElement = "StructureSwitch";
	// ATTRIBUTS
	_expressionATester; // String
	_ancienneExpressionATester; // String
	_editeur = editeur; // Editeur
divExpressionATester!: HTMLDivElement;
	 ...
	 */
	renameInformation(ancienNom: any, nouveauNom: string|null) {
	...
	afficher() {
		...
		this.divExpressionATester.addEventListener("focusout", (e: any) => {
			if (this._ancienneExpressionATester != this.expressionATester) {
				this._editeur.ajouterEvenement(
					new classes.EvenementEditionExpressionSwitch(
						this,
