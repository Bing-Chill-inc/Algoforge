...
import type { ElementParent } from "./ElementParent";
 ...
 */
export class Probleme extends ElementGraphique {
	static readonly typeElement = "Probleme";
	// ATTRIBUTS
	_libelle; // String
	_listeDonnes; // Liste d'Information
	_listeResultats; // Liste d'Information
	__elemParent!: ElementParent; // ElementParent (liste des enfants)
...
divResultatsEditable: any;
	...
	constructor(
		...
		listeDonnes = [],
		listeResultats = [],
		elemParent = new classes.ElementParent(),
	) {
	 ...
	 */
	set _elemParent(value: ElementParent) {
		this.__elemParent = value;
	}
	 ...
	 */
	replaceTexte(chaineAChercher: { [Symbol.replace](string: string,replaceValue: string): string; }, chaineARemplacer: string) {
	 ...
	 * @param {string} chaineARemplacer - La chaîne de remplacement.
	 */
	replaceTexteDonnee(chaineAChercher: { [Symbol.replace](string: string,replaceValue: string): string; }, chaineARemplacer: string) {
	 ...
	 * @param {string} chaineARemplacer - La chaîne de remplacement.
	 */
	replaceTexteResultat(chaineAChercher: { [Symbol.replace](string: string,replaceValue: string): string; }, chaineARemplacer: string) {
	...
	afficher() {
		...
		this.divDonneesEditable.addEventListener("input", (e: any) => {
		...
		this.divDonneesEditable.addEventListener("focusout", (e: any) => {
			if (this._listeDonnes == this.listeDonnes) return;
			this._editeur.ajouterEvenement(
				new classes.EvenementEditionDonneesProbleme(
					this,
					this._listeDonnes,
					this.listeDonnes,
				),
		...
		this.divNom.addEventListener("input", (e: any) => {
			editeur.adjustFontSize(this.divNom);
		});

		this.divNom.addEventListener("focusout", (e: any) => {
			...
			this._editeur.ajouterEvenement(
				new classes.EvenementEditionLibelleProbleme(
					this,
					this._libelle,
					this.libelle,
				),
		...
		this.divResultatsEditable.addEventListener("input", (e: any) => {
			if (verbose) console.log(e);
			if (this.divResultatsEditable.textContent == "") {
				for (let accolade of this.getElementsByClassName(
					"resultat",
		...
		this.divResultatsEditable.addEventListener("focusout", (e: any) => {
			if (this._listeResultats == this.listeResultats) return;
			this._editeur.ajouterEvenement(
				new classes.EvenementEditionResultatsProbleme(
					this,
	...
	decomposerAutrePlan(openAfterCreation = true) {
		...
		if (this._sousPlan != null) {
			...
			this._sousPlan = new classes.SousPlanTravail(this);
			this._espacePrincipalWrapper!.appendChild(this._sousPlan);

			let json = [this.toJSON()];

			const appliquerDecalage = (elem: { typeElement: any; abscisse: number; ordonnee: number; libelle: any; listeDonnes: any; listeResultats: any; enfants: any; estDecomposeAilleurs: boolean; }|{ typeElement: any; abscisse: number; ordonnee: number; libelle: any; listeDonnes: any; listeResultats: any; enfants: any[]; estDecomposeAilleurs?: undefined; }) => {
				elem.abscisse =
			...
			for (let elem of listeElemEnfantsARetirer) {
				if (elem != this && elem instanceof classes.ElementGraphique) {
	...
	genererOptionsContextuelles(editeur: any) {
		...
		if (this._sousPlan == null) {
			let decomposerAutrePlan = new classes.ElementMenu(
				"Décomposer ailleurs",
				() => {
			...
			let decomposerIci = new classes.ElementMenu("Décomposer ici", () => {
		...
		let exporter = new classes.ElementMenuCompose("Exporter le sous-arbre", () => {
