 ...
 */
export class PlanTravail extends HTMLElement {
	// ATTRIBUTS
	_editeur = editeur; // Editeur d'algorithme
	 ...
	 */
	static FiltrerElementsGraphique(listeElementGraphique: any[]|HTMLCollection, typeRechercher: ElementGraphique) {
		let nouvelleListe = [];
	 ...
	 */
	static trierElementsGraphique(listeElementGraphique: string|any[]) {
	...
	exporterEnJSON() {
		...
		for (let element of this.children) {
			if (
				...
				element instanceof classes.ElementGraphique
			) {
	 ...
	 * @returns {Array} Le plan de travail sous forme JSON.
	 */
	exporterEnJSONSpecifier(listeElemEnfantsAConcerver: any) {
		let listeElementsSansParents = [];
		for (let element of this.children) {
			if (
				element._parent == null &&
	 ...
	 */
	chargerDepuisJSON(corpsJSON: undefined, cancellable = true, dico = true) {
		...
		for (let element of corpsJSON) {
			switch (element.typeElement) {
				case "Probleme":
					let probleme = new classes.Probleme(
						element.abscisse,
						element.ordonnee,
						element.libelle,
					);
					for (let donnee of element.listeDonnes) {
						probleme._listeDonnes.push(new classes.Information(donnee));
					}
					for (let resultat of element.listeResultats) {
						probleme._listeResultats.push(
							new classes.Information(resultat),
						);
					}
					this.appendChild(probleme);
					if (cancellable)
						this._editeur.ajouterEvenement(
							new classes.EvenementCreationElement(probleme, this),
						);
					...
					break;
				case "Procedure":
					let procedure = new classes.Procedure(
						element.abscisse,
						element.ordonnee,
						element.libelle,
					);
					for (let donnee of element.listeDonnes) {
						procedure._listeDonnes.push(new classes.Information(donnee));
					}
					for (let resultat of element.listeResultats) {
					...
					if (cancellable)
						this._editeur.ajouterEvenement(
							new classes.EvenementCreationElement(procedure, this),
						);
					for (let enfant of this.chargerDepuisJSON(
						element.enfants,
						cancellable,
				...
				case "StructureSi":
					...
					let structureSi = new classes.StructureSi(
						element.abscisse,
						element.ordonnee,
						listeConditionsSi,
					);
					...
					if (cancellable)
						this._editeur.ajouterEvenement(
							new classes.EvenementCreationElement(structureSi, this),
						);
				...
				case "StructureSwitch":
					...
					let structureSwitch = new classes.StructureSwitch(
						element.abscisse,
						element.ordonnee,
					...
					if (cancellable)
						this._editeur.ajouterEvenement(
							new classes.EvenementCreationElement(structureSwitch, this),
						);
					...
					break;
				case "Condition":
					let condition = new classes.Condition(element.libelle);
					// condition.afficher();
				...
				case "StructureIterativeNonBornee":
					let structureIterativeNonBornee =
						new classes.StructureIterativeNonBornee(
							element.abscisse,
							element.ordonnee,
						);
					...
					if (cancellable)
						this._editeur.ajouterEvenement(
							new classes.EvenementCreationElement(
				...
				case "StructureIterativeBornee":
					let structureIterativeBornee = new classes.StructureIterativeBornee(
						element.abscisse,
						element.ordonnee,
				...
				case "ConditionSortie":
					let conditionSortie = new classes.ConditionSortie(
						element.abscisse,
						element.ordonnee,
					);
					...
					if (cancellable)
						this._editeur.ajouterEvenement(
							new classes.EvenementCreationElement(conditionSortie, this),
						);
	...
	effectuerDictionnaireDesDonnee() {
		...
		let lesInformations: any[] = [];
		for (let courantObjetGraphique of this.children) {
			if (courantObjetGraphique instanceof classes.ElementGraphique)
				lesInformations = [
					...lesInformations,
	...
	trouverToutLesElementsGraphiques() {
		let elements = [];
		for (let element of this.children) {
			if (element instanceof classes.ElementGraphique) {
	 ...
	 */
	ajouterElement(element: null, abscisse: string|number, ordonnee: string|number, estEnVW: boolean) {
		if (element == null) {
			return;
		}
		if (!estEnVW) {
		...
		this._editeur.ajouterEvenement(
			new classes.EvenementCreationElement(newElement, this),
		);
		return newElement;
	}
	 ...
	 */
	toutDeplacer(x: number, y: number) {
		for (let element of this.children) {
			if (element instanceof classes.ElementGraphique) {
