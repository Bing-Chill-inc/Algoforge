 ...
 * @extends {HTMLElement}
 */
export class ElementGraphique extends HTMLElement {
	// ATTRIBUTS
...
private _elemParent: any;
	 ...
	 */
	set _abscisse(value: number) {
		this.__abscisse = value;
	}
	 ...
	 */
	set _ordonnee(value: number) {
		this.__ordonnee = value;
	}
	...
	getDescendants(typeRechercher = ElementGraphique) {
		...
		return classes.PlanTravail.FiltrerElementsGraphique(
			listeDeMesDescendants,
			typeRechercher,
		);
	}
	...
	getAntescedants(typeRechercher = ElementGraphique) {
		...
		let listeDeMesAntescedants: any[] = [];
		if (parent !== null) {
	...
	genererOptionsContextuelles(editeur) {
		...
		if (this.peutEtreDecompose()) {
			lesOptions.push(
				new classes.ElementMenu("Décomposer", () => {
			...
			if (this._elemParent.nombreEnfants != 0) {
				lesOptions.push(
					new classes.ElementMenu("Délier tous les enfants", () => {
		...
		if (this._parent != null) {
			lesOptions.push(
				new classes.ElementMenu("Délier du parent", () => {
