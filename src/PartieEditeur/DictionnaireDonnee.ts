...
private _refreshBtn: any;
private _tableBody: any;
private _validInputs: any;
private _removeInputs: any;
private _inputName: any;
private _inputType: any;
private _inputSignification: any;
private _errorNameMsg: string;
private _lastErrorMsg: string;
private _errorTypeMsg: string;
private _errorSignificationMsg: string;
private _template: HTMLElement|null;
private _validRemove!: HTMLElement|null;
private _cancelRemove!: HTMLElement|null;
	 ...
	 */
	#handleSelectedRow(htmlTarget: { localName: string; parentNode: { id: any; }; id: any; }) {
		if (
			this._lastRow != "" &&
	 ...
	 */
	#insertSelectedRowTextOnInputs(rowId: string) {
	 ...
	 * @param {String} type
	 */
	#updateTypeList(type: string) {
		if (!classes.Type.allTypes.includes(type)) {
			classes.Type.allTypes.push(type);
		}
	}
	 ...
	 */
	#updateTypeInformation(type: string, information: { _type: any; _nom: string|number; }) {
		if (type !== "Non Défini") {
	 ...
	 * @param {Information} information
	 */
	#updateSignificationInformation(signification: string, information: { _signification: any; _nom: string|number; }) {
	 ...
	 * @param {String} signification
	 */
	#checkSignificationInput(signification: string) {
	...
	#populateTypeChoice() {
		...
		classes.Type.allTypes.forEach((type) => {
			this.querySelector("datalist")!.innerHTML +=
	...
	#launchInputTypeListener() {
		this._inputType.addEventListener("input", (e: { target: { value: any; }; }) => {
		...
		this._inputType.addEventListener("focus", (e: { target: { value: any; }; }) => {
			this.#checkTypeInput(e.target.value);
			document.getElementById("inputs-error")!.textContent =
				this._errorTypeMsg;
			this._lastErrorMsg = this._errorTypeMsg;
	...
	#launchInputSignificationListener() {
		this._inputSignification.addEventListener("input", (e: { target: { value: any; }; }) => {
		...
		this._inputSignification.addEventListener("focus", (e: { target: { value: any; }; }) => {
			this.#checkSignificationInput(e.target.value);
			document.getElementById("inputs-error")!.textContent =
				this._errorSignificationMsg;
			this._lastErrorMsg = this._errorSignificationMsg;
	...
	#launchInputNameListener() {
		this._inputName.addEventListener("input", (e: { target: { value: any; }; }) => {
		...
		this._inputName.addEventListener("focus", (e: { target: { value: any; }; }) => {
			this.#checkNameInput(e.target.value);
			document.getElementById("inputs-error")!.textContent =
				this._errorNameMsg;
			this._lastErrorMsg = this._errorNameMsg;
	...
	#launchListeners() {
		setTimeout(() => {
			...
			this._closeBtn.addEventListener("click", (e: { stopPropagation: () => void; }) => {
				e.stopPropagation();
				this.fermer();
			});

			this._refreshBtn.addEventListener("click", (e: { stopPropagation: () => void; }) => {
				e.stopPropagation();
				setTimeout(() => {
					this.fermer();
					this.ouvrir();
			...
			this._tableBody.addEventListener("click", (e: { target: any; }) => {
	 ...
	 * @returns {Boolean}
	 */
	AjouterUneVariable(uneInformation: { _nom: string; _type: any; }) {
		if (uneInformation._nom == this.VARIABLE_SUPPR) {
			return false;
		}
		let reussis = false;
		const nameInformation = uneInformation._nom.trim();
		if (uneInformation instanceof classes.Information) {
	 ...
	 */
	retirerInformationsAbsentes(listeInformations: string|never[]) {
		for (let info of this._mesInformations) {
	 ...
	 * @returns {Boolean}
	 */
	retirerUneInformation(nameVariable: any) {
	 ...
	 * @returns {Boolean}
	 */
	TypeCompatible(type1: undefined, type2: undefined) {
	 ...
	 * @returns {String}
	 */
	convertionVariable(informationUne: any, InformationDeux: any) {
		// undefined
		// String
		// Char
		// unsigned double
	 ...
	 * @returns {String}
	 */
	getTypeLePlusBasEnCommun(type1: any, type2: any) {
		let courant = type1;
		while (true) {
			if (!courant) {
				break;
	 ...
	 * @returns {Boolean}
	 */
	containInformation(nameInformation: string) {
		let trouver = false;
		this._mesInformations.forEach((element) => {
	 ...
	 * @returns {Information}
	 */
	getInformation(nameInformation: string) {
	 ...
	 * @returns {Boolean}
	 */
	renameInformation(nameVariable: any, newName: string) {
		let resultat = false;
		if (newName == "") {
			resultat = this.retirerUneInformation(nameVariable);
	 ...
	 * @returns {Boolean}
	 */
	changeSignification(nameVariable: any, nouvelleSignification: any) {
		let resultat = false;
		this._mesInformations.forEach((element) => {
			if (element._nom == nameVariable) {
	 ...
	 * @returns {Boolean}
	 */
	changeType(nameVariable: any, newType: any) {
		let resultat = false;
		this._mesInformations.forEach((element) => {
			if (element._nom == nameVariable) {
	 ...
	 * @returns {Boolean}
	 */
	nomCorrecte(nameVariable: string) {
		let regex = /^[a-zA-ZÀ-ÿ\(\)\[\]][a-zA-ZÀ-ÿ0-9\s\(\)\[\]]*$/g;
	 ...
	 * @param {JSON} json
	 */
	chargerDepuisJSON(json: { types: {}; signification: {}; }) {
	 ...
	 * @returns {String}
	 */
	csvToMarkdown(csvString: string) {
		const rows = csvString.split("\n").filter((row: any) => row); // On sépare les lignes et ne gardons pas les lignes vides
		let markdown = "";
