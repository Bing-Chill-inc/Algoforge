	...
	update() {
		if (this._elemGraphiqueParent._elemParent.nombreEnfants > 1) {
			...
			if (distance > 15 || !(angle > Math.PI / 4 && angle < (3 * Math.PI) / 4)) {
				...
				this._ligneDecomp.setDebut(ancreParent.abscisse, ancreParent.ordonnee);
				this._ligneDecomp.setFin(ancreEnfant.abscisse, ancreEnfant.ordonnee);
				this._ligneH.setDebut(ancreParent.abscisse, ancreParent.ordonnee);
				this._ligneH.setFin(ancreEnfant.abscisse, ancreEnfant.ordonnee);
				this._ligneV.setDebut(ancreEnfant.abscisse, ancreEnfant.ordonnee);
