class ThemeEditeur extends HTMLOptionElement {
  ...
  appliquer() {
    ...
    document.body.style.setProperty("--fgColorDisabled", this.fgColorDisabled);
    ...
    const ruleSelectorCompte = "menu-compte-element > div.img";
    ...
    for (let i = 0; i < sheet.cssRules.length; i++) {
      ...
      if (sheet.cssRules[i].selectorText === ruleSelectorCompte) {
        ruleToEdit = sheet.cssRules[i];
        ruleToEdit.style.backgroundImage = `url(assetsDynamiques/erreurs.php?fgColor=${this.fgColor.substring(
          1
        )})`;
    ...
    document.querySelector(
      "img#boutonProbleme"
    ).src = `assetsDynamiques/mini/probleme.php?fgColor=${this.fgColor.substring(
      1
    )}`;

    document.querySelector(
      "img#boutonProcedure"
    ).src = `assetsDynamiques/mini/procedure.php?fgColor=${this.fgColor.substring(
      1
    )}`;

    document.querySelector(
      "img#boutonStructureSi"
    ).src = `assetsDynamiques/mini/structureSi.php?fgColor=${this.fgColor.substring(
      1
    )}`;

    document.querySelector(
      "img#boutonStructureSwitch"
    ).src = `assetsDynamiques/mini/structureSwitch.php?fgColor=${this.fgColor.substring(
      1
    )}`;
    ...
    ).src = `assetsDynamiques/mini/conditionSortie.php?fgColor=${this.fgColor.substring(
      1
    )}`;

    document.querySelector(
      "img#boutonLien"
    ).src = `assetsDynamiques/mini/lien.php?fgColor=${this.fgColor.substring(
      1
    )}`;
    ...
    document.querySelector(
      "img#boutonUndo"
    ).src = `assetsDynamiques/mini/undo.php?fgColor=${this.fgColor.substring(
      1
    )}`;

    document.querySelector(
      "img#boutonRedo"
    ).src = `assetsDynamiques/mini/redo.php?fgColor=${this.fgColor.substring(
      1
    )}`;

    this._logoAlgoForge.src = `assetsDynamiques/AlgoForge.php?fgColor=${this.fgColor.substring(
      1
    )}`;
