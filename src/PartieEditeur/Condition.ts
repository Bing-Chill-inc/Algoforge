...
export class Condition extends HTMLElement {
	static readonly typeElement = "Condition";
	// ATTRIBUTS
	__elemParent!: ElementParent; // ElementParent (liste des enfants)
	__structure!: EventTarget|null; // StructureAlternative qui contient cette condition
	_editeur = editeur; // Editeur
	_ancienLib;
divLibelle: any;
flecheGauche!: HTMLSpanElement;
flecheDroite!: HTMLSpanElement;
divAjouterAGauche!: HTMLDivElement;
divAjouterADroite!: HTMLDivElement;
	 ...
	 */
	set _structure(value: any) {
		this.__structure = value;
	}
	...
	afficher() {
		...
		this.divLibelle.addEventListener("focusout", (e: any) => {
			if (this._structure instanceof classes.StructureSi) {
			...
			if (this._ancienLib != this._libelle) {
				this._editeur.ajouterEvenement(
					new classes.EvenementEditionLibelleCondition(
						this,
						this._ancienLib,
						this._libelle,
					),
		...
		this.divLibelle.addEventListener("input", (e: any) => {
		...
		this.divAjouterAGauche.addEventListener("click", (e: any) => {
		...
		this.divAjouterADroite.addEventListener("click", (e: any) => {
			this._structure.ajouterConditionParRapportA(this, 1);
		});
	 ...
	 */
	supprimer() {
		this._editeur.ajouterEvenement(new classes.EvenementSuppressionElement(this));
