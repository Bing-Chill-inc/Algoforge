...
import type { StructureIterativeBornee } from "./StructureIterativeBornee";
 ...
 * @extends {HTMLElement}
 */
export class InviteBornesPourSI extends HTMLElement {
	_structureIterative;
	_structureIterativeBornee;
	_structureIterativeNonBornee;
	_editeur = editeur; // Editeur
indice: HTMLInputElement;
borneInf: HTMLInputElement;
borneSup: HTMLInputElement;
selectCroissant: HTMLSelectElement;
pas: HTMLInputElement;
button: HTMLButtonElement;
nBorne: HTMLButtonElement;
	 ...
	 */
	constructor(
		uneStructureIterative: StructureIterativeBornee|StructureIterativeNonBornee|undefined,
		oldIndice = "",
		oldBorneInf = "",
		oldBorneSup = "",
		oldPas = "",
		...
		if (uneStructureIterative instanceof classes.StructureIterativeBornee) {
			...
			uneStructureIterative instanceof classes.StructureIterativeNonBornee
		) {
			this._structureIterativeNonBornee = uneStructureIterative;
			this._structureIterativeBornee = new classes.StructureIterativeBornee();
			if (verbose)
				console.log(
		...
		this.nBorne.addEventListener("click", (e: any) => {
			this.nonBorne();
		});
	...
	valider(estAnnulation = false) {
		...
		ancienneStructure.estBornee =
			this._structureIterative instanceof classes.StructureIterativeBornee;
		...
		if (!estAnnulation) {
			this._editeur.ajouterEvenement(
				new classes.EvenementEditionStructureIterative(
		...
		if (this._structureIterative instanceof classes.StructureIterativeBornee) {
			...
			this._structureIterative instanceof classes.StructureIterativeNonBornee
		) {
