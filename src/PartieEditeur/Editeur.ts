...
import notImplementedHtml from "../modales/notimplemented.html" with { type: "text" };
import shortcutsHtml from "../modales/shortcuts.html" with { type: "text" };
import type { EvenementComposite } from "./EvenementEdition/EvenementComposite";
import type { EvenementPlaceholder } from "./EvenementEdition/EvenementPlaceholder";
 ...
 * @extends {HTMLElement}
 */
export class Editeur extends HTMLElement {
	// ATTRIBUTS
	_logoAlgoForge = null;
	_themeSelect = null;
	_indicateurZoom = new classes.IndicateurZoom();
	_currentTool = -1;
	_listeTools = [];
	_typesElements = [];
	...
	_planActif = null;
	_selection = new classes.Selection();
	_selectionRectangle = new classes.SelectionRectangle();
	...
	_clickSound = new Audio(clickSoundUrl);
	...
	_dictionnaireDesDonnees = new classes.DictionnaireDonnee();
	_bibliotheque = new classes.Bibliotheque();
	...
	constructor() {
		...
		const shortcuts = shortcutsHtml
			.replaceAll("{metakey}", this._toucheMeta)
			.replaceAll("{altkey}", this._toucheMetaAlt);
		this._modaleRaccourcisClavier = new classes.FenetreModale(shortcuts);
		this.appendChild(this._modaleRaccourcisClavier);

		this._modaleAPropos = new classes.FenetreModale(aboutHtml);
		this.appendChild(this._modaleAPropos);

		this._modaleNoPaste = new classes.FenetreModale(
			noPasteHtml.replaceAll("{metakey}", this._toucheMeta),
		);
		this.appendChild(this._modaleNoPaste);

		this._modaleNonImp = new classes.FenetreModale(notImplementedHtml);
		...
		this._typesElements.push(classes.Lien);
		this._typesElements.push(classes.Probleme);
		this._typesElements.push(classes.Procedure);
		this._typesElements.push(classes.StructureSi);
		this._typesElements.push(classes.StructureSwitch);
		this._typesElements.push(classes.StructureIterativeNonBornee);
		this._typesElements.push(classes.StructureIterativeBornee);
		this._typesElements.push(classes.ConditionSortie);
				...
				fileInput.addEventListener("change", () => {
					...
					reader.onload = () => {
						try {
							...
							).some((element) => element instanceof classes.ElementGraphique);
		...
		this._menuDeroulantEdition!.ajouterElementMenu(
			new classes.ElementMenu("Importer", () => {
		...
		this._menuDeroulantEdition!.ajouterElementMenu(
			new classes.ElementMenu("Prettify", () => {
		...
		this._menuDeroulantAide!.ajouterElementMenu(
			new classes.ElementMenu("Raccourcis clavier", () => {
		...
		this._menuDeroulantAide!.ajouterElementMenu(
			new classes.ElementMenu("À propos", () => {
			...
			try {
				...
				const appliquerDecalage = (elem: { abscisse: string; ordonnee: string; enfants: any[]; typeElement: string; conditions: any; }) => {
					...
					elem.ordonnee =
						...
						"vw";
					if (elem.enfants) {
						elem.enfants.forEach((enfant: any) => {
					...
					if (
						...
						for (let condition of elem.conditions) {
							condition.enfants.forEach((enfant: any) => {
								appliquerDecalage(enfant);
							});
						}
					}
		...
		this.addEventListener("click", (e) => {
			...
			while (
				!(maTarget instanceof classes.ElementGraphique) &&
				!(maTarget instanceof classes.PlanTravail) &&
				!(maTarget instanceof classes.Condition) &&
				maTarget !== null
			) {
			...
			if (verbose) console.log(maTarget);
			if (maTarget instanceof classes.PlanTravail) {
				...
				if (
					...
					if (elemPose instanceof classes.StructureIterativeBornee) {
				...
				if (this._currentTool === 0) {
					if (this._pointePrecedementLien === null) {
						...
						if (
							this._pointePrecedementLien instanceof classes.Condition &&
							maTarget instanceof classes.Condition
						) {
						...
						if (enfantRelation instanceof classes.Condition) {
		...
		this.addEventListener("mousedown", function (e) {
			...
			if (maTarget instanceof classes.Condition) {
			...
			if (maTarget instanceof classes.PlanTravail && this._currentTool == -1) {
				...
				for (let element of this._selection.getElementsSelectionnes()) {
					this._evenementDeplacement.ajouterElementDeplace(
						new classes.EvenementDeplacementElement(element),
					);
				}
			}
		});
	 ...
	 * @returns {void}
	 */
	chargerDepuisJSON(json: any) {
		this.ajouterEvenement(new classes.EvenementPlaceholder()); // Marqueur de début d'événements
		...
		if (
			...
			lesElements[0] instanceof classes.StructureIterativeBornee
		) {
	...
	selectAll() {
		...
		for (let elem of this._planActif!.trouverToutLesElementsGraphiques()) {
			if (elem instanceof classes.ElementGraphique)
				this._selection.selectionnerElement(elem);
		}
	}
	...
	obtenirEnfantsElementParentDansOrdre(elementParent) {
		...
		return elementParent._listeElementsEnfants
			...
			.filter((element) => element instanceof classes.ElementGraphique);
	}

	creerNoeudMiseEnPageDepuisElement(element) {
		const noeud = {
			typeElement: element.constructor.typeElement,
			abscisse: element._abscisse,
			ordonnee: element._ordonnee,
			__elementReference: element,
		};
		if (element instanceof classes.Probleme) {
		...
		if (element instanceof classes.StructureAlternative) {
			noeud.conditions = [];
	...
	creerForetMiseEnPageDepuisPlan(planTravail) {
		...
		const racines = Array.from(planTravail.children).filter(
			(element) => element instanceof classes.ElementGraphique && element._parent == null,
		);
		return this.creerForetMiseEnPageDepuisElements(racines);
	}

	collecterDeplacementsNoeudMiseEnPage(noeud, deplacements) {
		if (noeud?.__elementReference instanceof classes.ElementGraphique) {
			deplacements.push({
	...
	prettifyPlanActif(options = {}) {
		...
		const evenementPrettify = new classes.EvenementDispositionAutomatique(deplacements);
	...
	obtenirLargeurNoeudMiseEnPage(noeud, config) {
		...
		if (noeud.typeElement === "ConditionSortie") {
			if (
				...
				noeud?.__elementReference instanceof classes.ElementGraphique
			) {
	...
	problemeSansEntreesSorties(noeud) {
		...
		if (elementRef instanceof classes.Probleme) {
			return (
	...
	calculerMiseEnPageNoeud(noeud, config) {
		...
		if (
			...
			for (let i = 0; i < conditions.length; i++) {
				...
				if (noeud.__elementReference instanceof classes.StructureAlternative) {
		...
		fileInput.addEventListener("change", () => {
			...
			reader.onload = () => {
				try {
					if (selectedFile.size > 5000000) {
						alert(
							"Le fichier est trop volumineux (maximum 5 MegaOctets).",
						);
						return;
					}
					const algoInterprete = this.interpreterFichierAlgorithme(
	 ...
	 */
	exporterSVG(nodeToCopy: string|null, download = true, isJSON = false) {
		...
		let planExport = new classes.PlanTravail();
	 ...
	 * @param {boolean} [isJSON=false] - Indique si le nœud est au format JSON.
	 */
	exporterJPG(nodeToCopy: Element, download = true, isJSON = false) {
	 ...
	 * @param {boolean} [isJSON=false] - Indique si le nœud est au format JSON.
	 */
	exporterPNG(nodeToCopy: Element, download = true, isJSON = false) {
		this.createBitmapImageFromSvg("png", nodeToCopy, download, isJSON);
	}

	adjustFontSize(el: Element, minFontSize = 0.45, maxFontSize = 1.2) {
