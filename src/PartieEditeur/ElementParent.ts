...
import type { StructureSi } from "./StructureSi";
...
export class ElementParent {
	// ATTRIBUTS
	__listeElementsEnfants!: never[]; // Liste contient les ElementGraphiques et les Lignes
	__proprietaire: any; // ElementGraphique
	 ...
	 */
	constructor(proprietaire: any, listeElementsEnfants = []) {
	...
	get _listeElementsEnfants() {
		return this.__listeElementsEnfants;
	}
	 ...
	 */
	set _listeElementsEnfants(value: never[]) {
		this.__listeElementsEnfants = value;
	}
	 ...
	 */
	set _proprietaire(value: any) {
		this.__proprietaire = value;
	}
	 ...
	 */
	lierEnfant(elementAAjouter: StructureSi, provientAnnulation = false) {
		if (!provientAnnulation)
			this._editeur.ajouterEvenement(
				new classes.EvenementLiaison(this._proprietaire, elementAAjouter),
			);
		if (elementAAjouter instanceof classes.ElementGraphique) {
	 ...
	 * @param {boolean} [provientAnnulation=false] - Indique si l'action provient d'une annulation.
	 */
	delierEnfant(elementASupprimer: ElementGraphique, provientAnnulation = false) {
		if (!provientAnnulation)
			this._editeur.ajouterEvenement(
				new classes.EvenementSuppressionLiaison(
					this._proprietaire,
					elementASupprimer,
				),
			);
	 ...
	 */
	creerLienAdequat(elemGraphiqueParent: { _elemParent: { _listeElementsEnfants: string|any[]; }; }, elemGraphiqueEnfant: ElementGraphique, espaceTravail: any) {
		let nouveauLien;
		if (
			elemGraphiqueParent instanceof classes.Probleme ||
			elemGraphiqueParent instanceof classes.Procedure
		) {
			nouveauLien = new classes.LienCompositionProbleme(
				elemGraphiqueParent,
				elemGraphiqueEnfant,
				espaceTravail,
			);
		} else if (
			elemGraphiqueParent instanceof classes.StructureIterativeBornee ||
			elemGraphiqueParent instanceof classes.StructureIterativeNonBornee
		) {
			nouveauLien = new classes.LienTriple(
				elemGraphiqueParent,
				elemGraphiqueEnfant,
				espaceTravail,
			);
		} else if (elemGraphiqueParent instanceof classes.Condition) {
			if (elemGraphiqueParent._elemParent._listeElementsEnfants.length == 0) {
				nouveauLien = new classes.LienDroit(
					elemGraphiqueParent,
					elemGraphiqueEnfant,
					espaceTravail,
				);
				...
				for (const lien of elemGraphiqueParent._elemParent._listeElementsEnfants) {
					if (lien.ligne instanceof classes.LienDroit) {
						lien.ligne.supprimer();
						lien.ligne = new classes.LienTriple(
							elemGraphiqueParent,
							lien.element,
							espaceTravail,
						);
