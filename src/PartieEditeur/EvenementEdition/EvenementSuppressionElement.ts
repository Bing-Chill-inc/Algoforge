 ...
 */
export class EvenementSuppressionElement extends EvenementEdition {
	// ATTRIBUTS
	 ...
	 */
	constructor(elementConcerne: { parentNode: any; _parent: any; getEnfantsParCondition: () => never[]; getEnfants: () => any; }) {
		super(elementConcerne);
		this._planTravail = elementConcerne.parentNode;
		this._parent = elementConcerne._parent;
		if (elementConcerne instanceof classes.StructureAlternative) {
			this._listeEnfants = elementConcerne.getEnfantsParCondition();
		} else if (elementConcerne instanceof classes.ElementGraphique) {
			for (let enfant of elementConcerne.getEnfants()) {
				this._listeEnfants.push([elementConcerne, enfant]);
			}
		} else if (elementConcerne instanceof classes.Condition) {
			for (let enfant of elementConcerne.getEnfants()) {
				this._listeEnfants.push([elementConcerne, enfant]);
			}
		}
