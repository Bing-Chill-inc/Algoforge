 ...
 */
export class StructureIterativeNonBornee extends StructureIterative {
	static readonly typeElement = "StructureIterativeNonBornee";
	// ATTRIBUTS -non-
	 ...
	 */
	constructor(abscisse: number|undefined, ordonnee: number|undefined) {
	...
	genererOptionsContextuelles(editeur: any) {
		...
		listeOptions.push(
			new classes.ElementMenu("Ajouter une condition d'arrêt", () => {
				if (verbose) console.log("Ajouter une condition d'arrêt");
				let nouveauIf = new classes.StructureSi(
				...
				let sortie = new classes.ConditionSortie(
