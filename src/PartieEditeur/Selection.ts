...
import type { RepresentationSelectionSimple } from "./RepresentationSelectionSimple";
...
export class Selection extends HTMLElement {
	_listeElementsSelectionnes: RepresentationSelectionSimple[];
	 ...
	 */
	selectionnerElement(element: ElementGraphique) {
		if (
			element instanceof classes.ElementGraphique &&
			!this.estSelectionne(element)
		) {
			let rep = new classes.RepresentationSelectionSimple(element, this);
	 ...
	 */
	selectionnerArbre(element: { peutEtreDecompose: () => any; getEnfants: () => any; _listeConditions: { children: any; }; }) {
	 ...
	 */
	deselectionnerElement(element: any) {
	...
	supprimerTout() {
		for (var selection of this._listeElementsSelectionnes) {
			let event = new classes.EvenementSuppressionElement(selection._element);
	 ...
	 */
	coin(topOuBottom: string, leftOuRight: string|undefined) {
	 ...
	 */
	moveAllSelectedElements(deplacementAbscisse: number, deplacementOrdonnee: number) {
		...
		for (var selection of this._listeElementsSelectionnes) {
			...
			selection.update();
			if (
				selection._element instanceof classes.Probleme ||
				selection._element instanceof classes.Procedure ||
				selection._element instanceof classes.StructureIterative
			) {
				selection._element._elemParent.updateAll();
			} else if (selection._element instanceof classes.StructureAlternative) {
	 ...
	 */
	estSelectionne(element: ElementGraphique) {
		for (var selection of this._listeElementsSelectionnes) {
